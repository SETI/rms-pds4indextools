...
- **A PDS4 label for that table**, saved with a ``.lblx`` extension. Just as
  ...
  is itself a small archive product, so it needs its own label. The index tool
...
You provide the interesting choices; the index tool handles the bookkeeping.
...
2. **Python and its installer, pip.** The index tool is a small Python program. If
   you can already run ``python`` and ``pip`` from a terminal, you are set;
   the next page walks through installing the index tool itself. You do not need to
   write any Python.
...
in order the first time. You will install the index tool, meet a small example
...
your choices, run the index tool, and read the results. A reference page near the
